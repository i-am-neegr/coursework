...
def test_simple_search_success(mock_load_data):
    ...
    # Проверка результата
    assert result == expected_result, f"Результат поиска должен совпадать с ожидаемым. Получено: {result}, Ожидалось: {expected_result}"
...
def test_simple_search_no_results(mock_load_data):
    ...
    # Проверка результата
    assert result == expected_result, f"Результат должен быть пустым JSON. Получено: {result}, Ожидалось: {expected_result}"
